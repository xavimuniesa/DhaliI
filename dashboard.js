...
function updateLocationData() {
    if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            // Altitud: mostrada a la targeta de la Terra. Molts dispositius (portàtils,
            // alguns mòbils) no retornen mai aquest valor.
            const earthBadge = document.getElementById('earth-status');
            if (alt !== null && alt !== undefined) {
                earthBadge.innerText = `ALTITUD: ${Math.round(alt)} m`;
            } else {
                earthBadge.innerText = "ALTITUD: N/D";
            }

            try {
                const url = `https://api.open-meteo.com/v1/forecast?latitude=${currentLat}&longitude=${currentLon}&current=temperature_2m,relative_humidity_2m,apparent_temperature&models=best_match`;
                ...
                if (data.current) {
                    ...
                    document.getElementById('feels-val').innerText = feels.toFixed(1);
                    document.getElementById('feels-meter').style.width = `${Math.min(Math.max((feels + 10) * 2, 0), 100)}%`;
                }
            } catch (e) {
